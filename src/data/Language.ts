...
const scripts = {
    en: {
        ...
        'overlay.main.help': 'Click the popup window to proceed to the next help.',
 ...
 * Fixed Update Loop: Designed a fixed-timestep processing structure for physics and simulation layers`,
    ...
    kr: {
        ...
        'overlay.main.help': '팝업 창을 클릭 해 다음 도움말로 넘어가세요.',
...
export class Language {
    ...
    forceSet(lang: LangType) {
        this.currentLang = lang

        if (SceneManager.self) {
            const mother = SceneManager.self.mother

            // Change all texts recursively
            this.updateTextRecursively(mother!, lang)
        }
    }

    updateTextRecursively(object: Object3D, lang: LangType) {
        const children = object.children || []
        children.forEach((child) => {
            if (child instanceof UIText) {
                child.updateText(lang)
            }
            this.updateTextRecursively(child, lang)
        })
    }   
}