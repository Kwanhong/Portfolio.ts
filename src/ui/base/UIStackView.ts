    ...
    updateLayout(fillScreen: boolean = true): void {
        ...
        for (const child of this.uiChildren) {
            ...
            if (text.textMesh) {
                ...
                text.setSize({ width: this.size.width - this.spacing * 2, height: -textHeight + 20 });
            } else if (button) {
        ...
        for (const child of this.uiChildren) {
            if (this.axis === 'vertical') {
                child.position.set(0, - this.size.height - this.spacing * 1.5, 0);
                ...
                child.position.set(offset - this.size.width / 2, this.size.height, 0);
